...
public class HashTable {
    ...
    private static class OwnHashTable {
        ...
        void insert(Entry entry) {
            ...
            ListNode head = this.table.get(index);
        ...
        void printTable() {
            for (int i = 0; i < this.size; i++) {
                var result = traverseAddress(this.table.get(i));
                var valueJoined = String.join("->", result.stream().map(Record::toString).toList());
                StringBuilder sb = new StringBuilder();
                sb.append("Address ").append(i).append(": ");
                sb.append(valueJoined);
    ...
    private static void javaHashTableExample() {
