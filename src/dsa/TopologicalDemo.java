    ...
    public static void main(String[] args) {
        ...
        for (var course : csCourses) {
            ...
            if (!course.prerequisite().isEmpty()) {
                for (var p : course.prerequisite()) {
                    graph.get(p).add(course.courseId());
                    inDegree.put(course.courseId(),
                            inDegree.get(course.courseId()) + 1);
                }
            }
        }
        // BFS => Queue
        Queue<Integer> queue = new LinkedList<>();
        // Select only inDegree 0
        inDegree.entrySet().stream().filter(e -> e.getValue() == 0)
                .forEach(e -> queue.offer(e.getKey()));
        List<Integer> traverseResult = new ArrayList<>();
        while (!queue.isEmpty()) {
            var courseId = queue.poll();
        ...
        // Check in-degree if any inDegree > 0 => cycle exists
        if (inDegree.values().stream().anyMatch(e -> e > 0)) {
            throw new IllegalArgumentException("Some courses are not valid due to cycle");
        }
        for (var id : traverseResult) {
            System.out.println("Studying " + referenceMap.get(id));
        }
    }
